...
export default function Match3Page() {
  ...
  async function handleClearMatch() {
    if (!confirm("Clear Match 3 and rematchmake?")) return;
    setLoading(true);
    try {
...
async function handleCreateMatches() {
  ...
  try {
    for (let i = 0; i < pairings.length; i++) {
      ...
      const matchId = `match-3-${i + 1}`; // dynamic match ID
    ...
    setMessage("✅ Manual Match 3 generated without duplication!");
    fetchMatchesAndRounds();
  } catch (err) {
    console.error(err);
