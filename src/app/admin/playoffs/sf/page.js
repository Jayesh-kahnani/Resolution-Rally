...
import { generateSemifinals } from "@/app/api/playoffs/route"; // server-side helper function (same pattern you've used before)
...
async function fetchMatches() {
  try {
    ...
    // Filter only Semifinals by day = 5 (number)
    const matchList = allMatches.filter((m) => Number(m.day) === 5);
    console.log("Semifinal matches:", matchList);
...
async function handleGenerateSemifinals() {
  if (teams.length < 4) {
    alert("Not enough teams to generate Semifinals (need 4).");
    return;
  }

  setLoading(true);
  try {
    // Generate SFs and wait until Firestore writes are done
    await generateSemifinals(teams);
    ...
    // fetch sf matches (day === 5)
    await fetchMatches();

    setMessage("✅ Semifinals generated!");
  } catch (err) {
    console.error("generate semifinals error:", err);
    setMessage("❌ Error generating Semifinals");
  } finally {
    setLoading(false);
  }
}
...
/** ---------------- Clear SFs ---------------- **/
  async function handleClearMatch() {
    if (!confirm("Clear semifinal matches and their rounds?")) return;
    setLoading(true);
    try {
      ...
      setMatches([]);
      setRounds({});
      setMessage("✅ Semifinals cleared");
    } catch (err) {
      console.error("clear semifinals error:", err);
      setMessage("❌ Error clearing Semifinals");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-8 bg-gray-50 rounded-xl shadow-lg">
      <h1 className="text-3xl font-extrabold mb-6 text-gray-800">Semifinals</h1>

      <div className="flex gap-4 mb-6">
        <button onClick={handleGenerateSemifinals} disabled={loading || matches.length > 0} className="bg-green-600 text-white px-5 py-2 rounded-lg shadow hover:bg-green-700 disabled:opacity-50">
          {loading ? "Generating..." : "Generate Semifinals"}
        </button>
