...
import { generateQuarterfinals } from "@/app/api/playoffs/route"; // server-side helper function (same pattern you've used before)
...
async function fetchMatches() {
  try {
    // fetch all matches first to debug
    const snap = await getDocs(collection(db, "matches"));
    const allMatches = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    console.log("All matches:", allMatches);

    // Filter only Quarterfinals by day = 4 (number)
    const matchList = allMatches.filter((m) => Number(m.day) === 4);
    console.log("Quarterfinal matches:", matchList);

    setMatches(matchList);

    // fetch rounds per match
    const roundsData = {};
    await Promise.all(
...
/** ---------------- Generate QFs ---------------- **/
async function handleGeneratequarterfinals() {
  if (teams.length < 8) {
    alert("Not enough teams to generate Quarter Finals (need 8).");
    return;
  }

  setLoading(true);
  try {
    // Generate QFs and wait until Firestore writes are done
    await generateQuarterfinals(teams);

    // fetch all matches (no query filter) and log to confirm
    const snap = await getDocs(collection(db, "matches"));
    const allMatches = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    console.log("All matches after generation:", allMatches);

    // fetch QF matches (day === 4)
    await fetchMatches();

    setMessage("✅ Quarter Finals generated!");
  } catch (err) {
    console.error("generate quarter finals error:", err);
    setMessage("❌ Error generating Quarter Finals");
  } finally {
    setLoading(false);
  }
}
...
async function handleEndMatch(matchId, teamAId, teamBId, totalA, totalB, rounds) {
  ...
  /** ---------------- Score Handling (client-side) ---------------- **/
  function handleScoreChange(matchId, roundId, teamKey, crit, val, rnum, pid = null) {
    ...
    setRounds((prev) => {
      ...
      const newRList = rList.map((r) => {
        ...
        if (r.roundNumber === 3) {
          ...
          if (!pid) return r; // must have participant id for participant-level rounds
          if (!newScores[teamKey]) newScores[teamKey] = {};
  ...
  /** ---------------- Save a single round's scores to Firestore ---------------- **/
...
/** ---------------- Clear QFs ---------------- **/
  async function handleClearMatch() {
    if (!confirm("Clear Quarterfinal matches and their rounds?")) return;
    setLoading(true);
    try {
      ...
      setMatches([]);
      setRounds({});
      setMessage("✅ Quarterfinals cleared");
    } catch (err) {
      console.error("clear quarter finals error:", err);
      setMessage("❌ Error clearing Quarter Finals");
    } finally {
      setLoading(false);
    }
  }

  /** ---------------- Compute totals (client) ---------------- **/
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-8 bg-gray-50 rounded-xl shadow-lg">
      <h1 className="text-3xl font-extrabold mb-6 text-gray-800">Quarterfinals</h1>

      <div className="flex gap-4 mb-6">
        <button onClick={handleGeneratequarterfinals} disabled={loading || matches.length > 0} className="bg-green-600 text-white px-5 py-2 rounded-lg shadow hover:bg-green-700 disabled:opacity-50">
        ...
        return (
          <div key={match.id} className="bg-white border rounded-lg p-6 mb-6 shadow hover:shadow-md transition">
            <h2 className="text-xl font-semibold text-gray-800 mb-4 flex justify-between items-center">
              <span>{match.teamA?.name} <span className="text-gray-500">vs</span> {match.teamB?.name}</span>
              ...
              if (r.roundNumber === 3) {
                return (
                  <div key={r.id} className="mb-4">
                    <p className="font-semibold text-blue-700 mb-3">Round 3 — Team Evaluation</p>
                    <div className="overflow-x-auto">
                      <table className="table-auto border-collapse border border-gray-300 w-full text-sm">
                        <thead>
                          <tr className="bg-gray-100">
                            <th className="border px-2 py-1">Team Name</th>
                            {criteria.map(c => <th key={c} className="border px-2 py-1">{c}</th>)}
                        ...
                        <tbody>
                          {["teamA","teamB"].map(tk => {
                            ...
                            const total = criteria.reduce((s,c) => s + Number(teamScores[c] || 0), 0);
                            return (
                              <tr key={tk}>
                                <td className="border px-2 py-1 font-semibold">{tk === "teamA" ? match.teamA?.name : match.teamB?.name}</td>
                                {criteria.map(c => (
                                  <td key={c} className="border px-2 py-1">
                                    <input type="number" min={0} max={ROUND_CRITERIA_MAX["round 3"][c.toLowerCase()]} value={teamScores[c] ?? 0} onChange={(e) => handleScoreChange(match.id, r.id, tk, c, e.target.value, r.roundNumber)} className="w-20 border px-2 py-1 rounded text-sm focus:ring-1 focus:ring-blue-400"/>
                                  </td>
                                ))}
                    ...
                    <div className="mt-3">
                      <button onClick={() => handleSaveScores(match.id, r.id)} className="bg-blue-600 text-white px-4 py-2 rounded-md shadow hover:bg-blue-700 transition">{savedRounds[r.id] ? "Saved!" : "Save Scores"}</button>
                    </div>
                  </div>
                );
              }

              // participant-level rounds 1 & 2
              return (
                <div key={r.id} className="mb-4">
                  <p className="font-semibold text-blue-700 mb-3">Round {r.roundNumber}</p>
                  {["teamA","teamB"].map(tk => {
                    const teamScores = r.scores?.[tk] || {};
                    if (!teamScores || Object.keys(teamScores).length === 0) return null;
                    const firstParticipant = Object.values(teamScores)[0] || {};
                    const partCriteria = Object.keys(firstParticipant).filter(k => !["id","name","role","teamId","rootId","totalScore"].includes(k));
                    return (
                      <div key={tk} className="mb-4">
                        <p className="font-semibold text-gray-700 mb-2">{tk === "teamA" ? match.teamA?.name : match.teamB?.name}</p>
                        <div className="overflow-x-auto">
                          <table className="table-auto border-collapse border border-gray-300 w-full text-sm">
                            <thead>
                              <tr className="bg-gray-100">
                                <th className="border px-2 py-1">Participant</th>
                                {partCriteria.map(c => <th key={c} className="border px-2 py-1">{c}</th>)}
                                <th className="border px-2 py-1">Total</th>
                              </tr>
                            </thead>
                            <tbody>
                              {Object.values(teamScores).map(p => {
                                const total = partCriteria.reduce((s,c) => s + Number(p[c] || 0), 0);
                                return (
                                  <tr key={p.id}>
                                    <td className="border px-2 py-1">{p.name}</td>
                                    {partCriteria.map(c => (
                                      <td key={c} className="border px-2 py-1">
                                        <input type="number" min={0} max={ROUND_CRITERIA_MAX[`round ${r.roundNumber}`][c.toLowerCase()]} value={p[c] ?? 0} onChange={(e) => handleScoreChange(match.id, r.id, tk, c, e.target.value, r.roundNumber, p.id)} className="w-20 border px-2 py-1 rounded text-sm focus:ring-1 focus:ring-blue-400"/>
                                      </td>
                                    ))}
                                    <td className="border px-2 py-1 font-semibold">{total}</td>
                                  </tr>
            ...
            <div className="mt-6 border-t pt-4">
              <p className="font-semibold text-lg text-purple-700 mb-2">Total Scores</p>
              <p className="mb-1">{match.teamA?.name}: {totalA}</p>
              <p className="mb-3">{match.teamB?.name}: {totalB}</p>
