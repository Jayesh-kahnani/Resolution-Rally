...
import { generateFinals } from "@/app/api/playoffs/route"; // server-side helper function (same pattern you've used before)
...
async function fetchMatches() {
  try {
    ...
    // Filter only Finals by day = 6 (number)
    const matchList = allMatches.filter((m) => Number(m.day) === 6);
    console.log("Final matches:", matchList);
...
async function handleGenerateFinals() {
  if (teams.length < 2) {
    alert("Not enough teams to generate Finals (need 2).");
    return;
  }

  setLoading(true);
  try {
    // Generate Finals and wait until Firestore writes are done
    await generateFinals(teams);
    ...
    // fetch finals matches (day === 6)
    await fetchMatches();

    setMessage("✅ Finals generated!");
  } catch (err) {
    console.error("generate finals error:", err);
...
/** ---------------- Clear finals ---------------- **/
  async function handleClearMatch() {
    if (!confirm("Clear final matches and their rounds?")) return;
    setLoading(true);
    try {
      ...
      setMatches([]);
      setRounds({});
      setMessage("✅ Finals cleared");
    } catch (err) {
      console.error("clear finals error:", err);
      setMessage("❌ Error clearing Finals");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-8 bg-gray-50 rounded-xl shadow-lg">
      <h1 className="text-3xl font-extrabold mb-6 text-gray-800">Finals</h1>

      <div className="flex gap-4 mb-6">
        <button onClick={handleGenerateFinals} disabled={loading || matches.length > 0} className="bg-green-600 text-white px-5 py-2 rounded-lg shadow hover:bg-green-700 disabled:opacity-50">
